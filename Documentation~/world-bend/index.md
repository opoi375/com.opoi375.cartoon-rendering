...
![完整效果：地面成弧、天空地平线下沉、云层向地平线聚拢](/worldbend/planet-look.png)

> 完整效果（`curvature = 0.005` 夸张演示）：地面弯成星球弧线，天空渐变的地平线带随之下沉贴合，体积云向地平线柔和聚拢而不消失，角色（含头发）整体随曲面下沉。
...
| 参数 | 默认 | 说明 |
| --- | --- | --- |
| `skyHorizonDistance` | 200 | 地面有效可见距离（米），推算天空地平线下沉角度，与地面边缘对齐 |
| `maxSkyDip` | 0.15 | 天空地平线最大下沉量（≈8.5°），防止极端曲率把整个渐变拉进天底颜色带 |
| `cloudBendScale` | 0.35 | 云层弯曲弱化系数。云比地面高，全强度下垂会快速沉出天际，弱化后向地平线柔和聚拢 |
| `cloudMaxDroop` | 300 | 云层最大下垂量（米），远处的云停在被压低但有界的壳层上 |

天空渐变只下沉**分带**；太阳、星星、2D 云保持真实方向（光照不变，二者不矛盾）。体积云的云底按采样点水平距离下垂， slab 相交底面同步下移防止裁切。
